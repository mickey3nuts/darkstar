...
function onUseAbilityRoll(caster, target, ability, total)
	...
	if (target:addCorsairRoll(caster:getMainJob(), caster:getMerit(MERIT_BUST_DURATION), EFFECT_BEAST_ROLL, effectpower, 0, duration, target:getID(), total, MOD_PET_ATTP) == false) then
		ability:setMsg(423);
	end
end;